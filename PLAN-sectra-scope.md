...
- **uidesc root must live under `"templates"` with key `"view"`.** In this VSTGUI version `VST3Editor::open()` calls `description->createView("view", ...)`, which only searches nodes under a top-level `"templates"` section whose key matches the requested name (see `UIDescription::createView`, uidescription.cpp). A bare top-level view node → null root → **no UI window at all**. Verified against the SDK `dataexchange` sample layout.
- **Crash fix (host-dependent): oversized audio blocks corrupted the heap.** `SpectrumAnalyzer::process()` advanced its trailing-N history with `history_.begin() + numSamples` where `history_` has size `fftSize_`. Hosts delivering blocks larger than the FFT size (Studio One; REAPER's default 512–1024 never triggered it) made that iterator land past the end → UB → `STATUS_HEAP_CORRUPTION` (0xc0000374) at playback start. Fix: `process()` now chunks input into ≤ fftSize frames and delegates to a private `processFrame()` (single-frame logic unchanged). Regression test: "L0: analyzer accepts blocks larger than the FFT size without corruption" (blocks of N, 2N, 4N; silence finiteness + 440 Hz peak location).
- **Crash fix #2 (dangling ScopeViews in `Controller::scopes_`).** Studio One crashed again post-fix #1 with the *identical* WER signature (c0000374, same ntdll offset), but device block size was 512 < fftSize 4096, so the oversized path never ran — a second independent bug. Mechanism: VSTGUI takes ownership of views returned from `createCustomView` (`viewContainer->addView(childView)`); on editor teardown `VST3Editor::close()` calls `delegate->willClose(this)` and then `getFrame()->removeAll(true)`, which deletes every view including our ScopeViews. The controller kept those raw pointers in `scopes_` forever and appended two more per reopen (hosts probe-open/close editors before the real open). During playback the processor keeps publishing spectra regardless of editor state, so `onDataExchangeBlocksReceived` wrote into freed memory → heap corruption. Fix: override `willClose(VST3Editor*)` to clear `scopes_` (views die exactly when willClose fires); plus a debug-only poison guard in `ScopeView` (destructor marks object dead; any later method access throws) so lifetime violations fail loudly instead of corrupting the heap. Regression tests: "L1: Sectra Scope controller drops scope views when the editor closes" (poison exception proves pre-fix write-to-freed-view) and "...keeps feeding only live views after an editor close/reopen cycle".
- **Crash fix #3 (the actual production killer): `dbA_{kNumCols, -120.f}` created a 2-element buffer.** Brace-init `{a, b}` on `std::vector` binds to the **initializer_list constructor**, not `(size, value)` — so `ScopeView::dbA_` held exactly 2 floats ({720.0f, −120.0f}) while `setData` unconditionally wrote all 720 columns through `dbA_[i]`: ~2856 bytes past the allocation on every audio block → guaranteed `STATUS_HEAP_CORRUPTION` at playback start, matching both Studio One WER dumps. This is why the crash reproduced even though fixes #1/#2 were correct and why unit probes corrupted the heap without any view deletion. Fix: plain member `std::vector<float> dbA_;` sized via `dbA_.assign(kNumCols, -120.f)` in the ctor body (paren-init as default member initializer is a most-vexing-parse function declaration under MSVC). Regression test: "L1: [iso] construct and destroy two bare ScopeViews" asserts `numStoredColumns() == kNumCols`. Debug-CRT `_CrtCheckMemory` + `/RTCs` localized it to `vector(1931) subscript out of range` inside `setData`; ASan was unusable in this environment (instant AV, no report).
...
- **UI layout fixes (post first successful run):** (a) *Only one scope window was visible* — delegate-created custom views never get their `"origin"` attribute applied by the factory (`createViewFromNode` only runs class-based attribute application on the result), so both ScopeViews landed at (0,0) stacked exactly. Fix: `Controller::createCustomView` reads `"origin"` from the node attributes itself and calls `setViewSize(CRect(origin, size))` per view. (b) *No captions under knobs* — `CKnob`/`CKnobBase` have no title-drawing support in this VSTGUI version (only checkbox/textbutton/textlabel creators handle `kAttrTitle`). Fix: six explicit `CTextLabel` nodes (`"class": "CTextLabel"` under `"CParamDisplay"`) centered under each knob at y=516, transparent, mouse-disabled.
- **uidesc edit trap:** an early caption patch accidentally inserted the label nodes inside the root `"attributes"` object → invalid JSON → VSTGUI silently failed to load the description → **no UI window at all** while playback stayed fine. Lesson: validate any hand-edited uidesc before building (duplicate child keys are legal for VSTGUI's parser but NOT for strict JSON parsers — check brace balance + structure instead), and confirm the packaged copy inside `build/VST3/<Config>/sectrascope.vst3/Contents/Resources/editor.uidesc`.
- **"Second scope invisible — per-child coordinate translation."** VSTGUI 4's `CViewContainer::drawRect` does **not** apply a per-child coordinate translation before calling `pV->drawRect()`. Children must render at their **absolute position** within the parent (standard controls like CKnob use `getViewSize().left/top` internally). Our `ScopeView::draw()` originally painted everything at local `(0,0)` — which partially overlapped scopeA's clip region (visible) but fell entirely outside scopeB's clip (completely clipped out → invisible). Fix: first line of `ScopeView::draw()` applies `CDrawContext::Transform tr(*pContext, CGraphicsTransform().translate(bounds.left, bounds.top))`, translating the context into our rect so all subsequent `(0,0)`-relative drawing lands correctly regardless of position. Also: each `ScopeView` strokes a distinct colored border (`setFrameColor`; blue for A / amber for B) so both panels remain visible even when silent (~−120 dB).
- **`ScopeView`** (`Source/ScopeView.{h,cpp}`): one instance per slot; holds a 720-float `dbA_` buffer (sized in ctor body — see crash fix #3) + a `LogFreqMap(20,20000,720)`. `draw(CDrawContext*)` paints bg #1E1E1E, left dB labels/margin, normal −120…0 grid every 6 dB (labels every 12) or ±12 balance grid with emphasized 0-line in balance mode, octave ticks at {20…10000} Hz, then a filled `CGraphicsPath` curve ({70,160,220,110}) with a brighter stroked top edge ({140,210,255,255}). Bin centers plotted at `(i+0.5)/720`.
- **Compile gotchas hit & fixed (MSVC x64):** (1) `SMTG_OVERRIDE` is not visible through VSTGUI's `cview.h` alone → used plain `override` in `ScopeView.h`; (2) sizing a vector member to (count, value) has NO working default-member-initializer form under MSVC: paren-init is a most-vexing-parse function declaration and brace-init binds to initializer_list (crash fix #3) → size it in the ctor body via `.assign()`; (3) unqualified `DataExchangeReceiverHandler` fails name lookup inside our namespace (no file-scope `using`) → fully qualified `Steinberg::Vst::DataExchangeReceiverHandler dataExchange_{this};`. Also: `TBool` is `Steinberg::TBool`, `IMessage` is `Steinberg::Vst::IMessage`.
